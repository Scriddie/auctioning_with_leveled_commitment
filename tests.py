...
class TestAuction(unittest.TestCase):
    ...
    def test_pure_commitment(self):
        ...
        self.assertListEqual(auction.bidding_factors.tolist(), [[1.0, 3.0],
                                                                [1.2, 2.0],
                                                                [3.0, 4.2]])


    def test_leveled_commitment(self):
        ...
        self.assertListEqual(pay_mat.tolist(), [[1.5, 0.0],
                                                [0.0, 3.0],
                                                [0.0, 0.0]])
        self.assertListEqual(market_prices.tolist(), [4.0, 6.0])
        self.assertListEqual(np.nan_to_num(opportunity_bids).tolist(), [[2.0, 0.0],
                                                                        [4.0, 5.0],
                                                                        [6.0, 7.0]])
        ...
        auction.update_bidding_factors(bids=opportunity_bids, market_prices=market_prices, pay_mat=pay_mat)
        self.assertListEqual(auction.bidding_factors.tolist(), [[1.0, 3.0],
                                                                [1.2, 2.0],
                                                                [3.0, 4.2]])
